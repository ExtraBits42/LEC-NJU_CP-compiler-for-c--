...
IrNode* translate_exp(Node* exp){
    ...
    if(exp->children->next == NULL && STREQ(exp->children->name, "ID")){
        ...
        IrNode* temp = new_irn(new_temp_name(), BASIC_T, NULL, NULL);
        IrStmt* p1 = new_ir_stmt(3, IR_C_ASSIGNOP, temp->name, ir_int->name);
        ADD_IR_STMT(p1);
        ...
        if(ir_n1->if_addr == 0){
            IrNode* temp1 = new_irn(new_temp_name(), BASIC_T, NULL, NULL);
            IrStmt* p0 = new_ir_stmt(3, IR_C_ASSIGNOP, temp1->name, "#0");
            IrStmt* p = new_ir_stmt(4, IR_MINUS, rst->name, temp1->name, ir_n1->name);
            ADD_IR_STMT(p0);
            ADD_IR_STMT(p);
        }
        else{
            ...
            IrStmt* p2 = new_ir_stmt(4, IR_MINUS, rst->name, temp1->name, rst->name);
            ADD_IR_STMT(p0);
            ADD_IR_STMT(p1);
            ADD_IR_STMT(p2);
        }
        ...
        */
        ...
        IrNode* temp3 = new_irn(new_temp_name(), BASIC_T, NULL, NULL);
        IrStmt* p0 = new_ir_stmt(3, IR_C_ASSIGNOP, temp3->name, str_offset);

        IrStmt* p1 = new_ir_stmt(4, IR_STAR, temp1->name, index->name, temp3->name);
...
void translate_cond(Node* exp, char* label_true, char* label_false){
    ...
    if(STREQ(exp->children->name, "NOT")){
        ...
        IrNode* temp2 = new_irn(new_temp_name(), BASIC_T, NULL, NULL);
        IrStmt* t1 = new_ir_stmt(3, IR_C_ASSIGNOP, temp2->name, "#0");
        ADD_IR_STMT(t1);

        IrStmt* p1 = new_ir_stmt(5, IR_IF_GOTO, op1->name, "!=", temp2->name, label_true);
        ADD_IR_STMT(p1);
...
void translate_args(Node* args){
    ...
    */
    ...
    IrNode* vname = translate_exp(exp);
    if(vname->if_addr == 1 && vname->ntype == BASIC_T){
        IrNode* temp1 = new_irn(new_temp_name(), BASIC_T, NULL, NULL);
        IrStmt* p1 = new_ir_stmt(3, IR_VP_ASSIGNOP, temp1->name, vname->name);
        IrStmt* p2 = new_ir_stmt(2, IR_ARG, temp1->name);
        ADD_IR_STMT(p1);
        ADD_IR_STMT(p2);
    }
    else{
