...
SymNode* vardec_anls(TypeNode* type, Node* vardec, int if_mnt){
    ...
    */
    ...
    SymNode* checker1 = check_semantic_error_3(id);
    if(checker1 != NULL) return NULL;
    // TypeNode* checker2 = check_semantic_error_17(type, id);
    // if(checker2 != NULL) return NULL;
...
void exp_anls(Node* exp){
    ...
    if(STREQ(exp->children->name, "ID")){
        ...
        //Exp->INT
        TypeNode* rst = new_basic_type_node(BASIC_TYPE_INT, CONSTANT);
    }
    else if(STREQ(exp->children->name, "FLOAT")){
        //Exp->FLOAT
        TypeNode* rst = new_basic_type_node(BASIC_TYPE_FLOAT, CONSTANT);
    }
        ...
        STREQ(exp->children->next->name, "PLUS") ||
